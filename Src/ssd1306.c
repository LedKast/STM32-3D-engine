...
void ssd1306_DrawLine(uint16_t x0, uint16_t y0, uint16_t x1, uint16_t y1, SSD1306_COLOR c) {
	...
	/* Check for overflow */
	// TODO неправильная отрисовка линий, которые выходят за размеры экрана
	// с отрицательной стороны перерисовывается с положительной
	// а с положительной лишними пиксельями обрисовывается нижняя строка
	if (x0 >= SSD1306_WIDTH) {
		x0 = SSD1306_WIDTH - 1;
	}
	if (x1 >= SSD1306_WIDTH) {
